...
export function issueTokens(user: User, res: any) {
  ...
  const refreshToken = jwt.sign({ id: user.id }, jwtRefreshSecret, { expiresIn: '2h' });
...
// GitHub Strategy
passport.use(
  new GitHubStrategy(github, async (accessToken, refreshToken, profile, done) => {
    try {
...
async function refreshToken(req, res) {
  ...
  try {
    ...
    const newAccessToken = jwt.sign({ id: user.id }, jwtSecret, { expiresIn: '30m' });

    // Optionally, issue a new refresh token
    const newRefreshToken = jwt.sign({ id: user.id }, jwtRefreshSecret, { expiresIn: '2h' });
